...
└── skills/
    └── java-api-review/
        └── SKILL.md`}</pre></div></article><article className="walk-step"><div className="step-text"><p className="step-number">PASSO 04</p><h3>Abra o agente: uma conversa com a IA</h3><p><strong>Agente</strong> é a tela em que você escreve para a IA e recebe uma resposta. No IntelliJ, essa conversa é o <strong>GitHub Copilot Chat</strong>.</p><div className="agent-guide"><p className="agent-guide-label">PRINT GUIADO · NO INTELLIJ</p><p><b>1.</b> Clique em <strong>GitHub Copilot</strong>, no topo da janela.</p><p><b>2.</b> Abra a opção <strong>Chat</strong>.</p><p><b>3.</b> Escreva sua pergunta na caixa de conversa que abrir.</p></div><p>Em um agente que suporta Skills nativamente, a mensagem seria:</p><div className="skill-call"><p>Use a Skill <strong>java-api-review</strong> para revisar a validação de pedido e propor testes.</p></div><div className="execution-flow"><span>Agente reconhece a tarefa</span><i>→</i><span>Abre <code>SKILL.md</code></span><i>→</i><span>Segue as regras e propõe os testes</span></div><p>No Copilot padrão do IntelliJ, não espere que a frase carregue sozinho o <code>SKILL.md</code>. Ele usa o arquivo <code>.github/copilot-instructions.md</code> do passo 02; por isso, no Chat você pede a revisão normalmente.</p></div><Figura src={imagens.sugestao} alt="Print do IntelliJ mostrando GitHub Copilot no topo do editor" legenda="Print guiado: no topo está o GitHub Copilot. Clique nele para abrir o Chat e conversar com a IA." /></article><article className="walk-step flipped"><div className="step-text"><p className="step-number">PASSO 05</p><h3>Veja uma sugestão do Copilot</h3><p>Agora use o Chat para pedir uma revisão simples. O texto abaixo <strong>não é o arquivo SKILL.md</strong>; é apenas um exemplo de código que o Copilot pode sugerir depois de ler as instruções do projeto.</p><div className="code-response-label">EXEMPLO DE RESPOSTA DO COPILOT · NÃO É A SKILL</div><pre>{`public boolean hasValidOrderId(String orderId) {
    return orderId != null       // não aceita valor nulo
        && !orderId.isBlank()    // não aceita campo vazio
        && orderId.matches("\\d+"); // aceita apenas números
}`}</pre><div className="code-meaning"><p><strong>Em palavras simples:</strong> o método verifica se o código do pedido existe, não está vazio e contém somente números.</p></div></div><Figura src={imagens.sugestao} alt="Imagem instrucional com uma sugestão de código Java do GitHub Copilot" legenda="Este print mostra uma sugestão do Copilot no editor — não mostra o conteúdo do arquivo SKILL.md." /></article><article className="walk-step flipped no-image"><div className="step-text"><p className="step-number">PASSO 06</p><h3>Pergunte, teste e revise</h3><p>Selecione o método e peça ao Copilot: <strong>“Explique esta validação e proponha três testes JUnit 5. Não altere arquivos.”</strong></p><div className="review-check"><Check size={17} /><span>Leia a proposta, compare com as regras da Skill, execute os testes e revise o diff antes de finalizar.</span></div></div><div className="prompt-card"><TerminalSquare size={22} /><p className="prompt-label">PROMPT PARA COPIAR</p><pre>{`Explique este método para alguém novo no projeto.
